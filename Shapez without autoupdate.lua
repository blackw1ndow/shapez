        ...
        imgui.Begin(u8'Shapez | '..version..' | Activation: F11##main', main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
           imgui.SetWindowPos(imgui.ImVec2(mX / 2.7, mY / 3), imgui.Cond.FirstUseEver)
            imgui.BeginChild('top', imgui.ImVec2(465, 95), true) 
                ...
                elseif imgui.Button(fa.ICON_FA_CROWN ..u8'', imgui.ImVec2(107, 78), imgui.SameLine()) and arenaMode then menu = 3
...
function checkBeforeLoad()
	...
	if timelock then sampAddChatMessage(tag ..'Бинды в випменю временно заблокированы на Arizona Supreme', -1) end
...
function ev.onSendCommand(int)
    if int == '/vip' and (arenaMode) then vip_window_state.v = not vip_window_state.v return false end
...
function ev.onShowDialog(id, style, title, b1, b2, text)
    ...
    if id == 2760 and not timelock and title:find("{BFBBBA}{ffffff}Arizona Supreme | {9758fc}VIP{ffffff} MENU") then return false end
	if id == 7760 and arenaMode and title:find("Выбор") then return false end
