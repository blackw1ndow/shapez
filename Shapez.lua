        ...
        imgui.Begin(u8'Shapez | '..version..' | Activation: F11##main', main_window_state, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse)
           imgui.SetWindowPos(imgui.ImVec2(mX / 3, mY / 3), imgui.Cond.FirstUseEver)
            imgui.BeginChild('top', imgui.ImVec2(465, 95), true) 
                ...
                elseif imgui.Button(fa.ICON_FA_CROWN ..u8'', imgui.ImVec2(107, 78), imgui.SameLine()) then
                    if config.messages.chat then 
                        msg("function is currently unavailable", "chat") elseif config.messages.console then msg("function is currently unavailable", "console") elseif config.messages.silent then 
                    end
            ...
            if menu == 1 then
                ...
                if config.main.shorten then
                    ...
                    if imgui.Button(fa.ICON_FA_SAVE ..u8'') then 
                        ...
                        sampRegisterChatCommand(config.commands.fhc, function(num) sampSendChat('/findhouse '..num) end) 
                        sampRegisterChatCommand(config.commands.fbc, function(num) sampSendChat('/findbiz '..num) end)
                        inicfg.save(config, "Shapez")
                    end
            ...
            if menu == 4 then
                imgui.PushItemWidth(110)
                if imgui.Combo(u8'Темы скрипта', temki, tmodes) then styles[temki.v]() if config.main.customlogo then if arizona then reqFunc("createlogo", "arizona") end end end
                imgui.PopItemWidth()
                imgui.SameLine(436)
                ...
                if config.main.customlogo then
                    if imgui.Button(fa.ICON_FA_TRASH ..u8'', imgui.SameLine()) then if arizona then reqFunc("deletelogo", "arizona") end end
